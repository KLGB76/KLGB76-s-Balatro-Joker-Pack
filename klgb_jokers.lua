...
SMODS.Joker{
    ...
    loc_txt = { -- local text
        name = 'The Seal',
        text = {
          'If {C:attention}first hand{} of round is a single',
          '{C:attention}face card{}, adds a {C:attention}random seal{} to that card',
        },
    },
    atlas = 'Jokers', --atlas' key
    rarity = 1, --rarity: 1 = Common, 2 = Uncommon, 3 = Rare, 4 = Legendary
    ...
    calculate = function(self, card, context)
        ...
        if context.before and #context.full_hand == 1 and context.full_hand[1]:is_face() and not context.full_hand[1].debuff and G.GAME.current_round.hands_played == 0 and not card.debuff then
            context.full_hand[1]:set_seal(SMODS.poll_seal({
                key = "seal-y",
                guaranteed = true,
               }), nil, true
            )
...
SMODS.Joker{
    ...
    loc_txt = { -- local text
        name = '4 21',
        text = {
          'This Joker gains {C:chips}+10{} Chips when',
          '{C:attention}hand played{} is exactly an {C:attention}Ace{}, a {C:attention}4{} and a {C:attention}2{}',
          'Additional {C:chips}+21{} Chips when playing that hand',
          '{s:0.7}{C:inactive}(Currently {C:chips}+#1#{}{C:inactive} Chips){}',
        },
    },
    atlas = 'Jokers', --atlas' key
    rarity = 2, --rarity: 1 = Common, 2 = Uncommon, 3 = Rare, 4 = Legendary
    ...
    loc_vars = function(self,info_queue,center)
        return {vars = {center.ability.extra.AddChips}}
    end,

    calculate = function(self, card, context)
        if context.before and #context.full_hand == 3 and not card.debuff and not context.blueprint then
            local has_ace = false
            local has_four = false
            local has_two = false
            for i=1, #context.full_hand do
                if context.full_hand[i]:get_id() == 14 then
                    if context.full_hand[i].config.center_key ~= 'm_stone' then
                        has_ace = true
                    end
                elseif context.full_hand[i]:get_id() == 4 then
                    if context.full_hand[i].config.center_key ~= 'm_stone' then
                        has_four = true
                    end
                elseif context.full_hand[i]:get_id() == 2 then
                    if context.full_hand[i].config.center_key ~= 'm_stone' then
                        has_two = true
                    end
                end
            end
            if has_ace and has_four and has_two then
                card.ability.extra.AddChips = card.ability.extra.AddChips + 10
                card.ability.extra.combo_played = true
                return {message = localize('k_upgrade_ex'),}
            end
        end
        ...
        if context.joker_main and card.ability.extra.AddChips > 0 and not card.debuff then
            local total_add = 0
            if card.ability.extra.combo_played == true then
                total_add = card.ability.extra.AddChips + 21
                card.ability.extra.combo_played = false
            else
                total_add = card.ability.extra.AddChips
            end
            return {
                card = card,
                mult_mod = total_add,
...
-- Magic Backpack
SMODS.Joker{
    key = 'backpack', --joker key
    loc_txt = { -- local text
        name = 'Magic Backpack',
        text = {
          'If {C:attention}first hand{} of round has only {C:attention}1{} card and',
          'no card is stored, {C:attention}destroy{} and {C:attention}store{} this card',
          'At the end of round, stored card',
          'permanently gains {C:chips}+15{} Chips',
          'Sell this card to {C:attention}create{} stored card',
          "{s:0.7}{C:inactive}(Card currently stored: #1#){}"
        },
    },
    atlas = 'Jokers', --atlas' key
    rarity = 1, --rarity: 1 = Common, 2 = Uncommon, 3 = Rare, 4 = Legendary
    ...
    loc_vars = function(self,info_queue,center)
        if center.ability.extra.storing == false then
            return {vars = {"None"}} 
        else
            local rank_suffix = center.ability.extra.stored_card:get_id()
            ...
            local total_bonus = 0
            if center.ability.extra.stored_card.config.center.key == "m_bonus" then
                total_bonus = center.ability.extra.stored_card.ability.perma_bonus + 30
            else
                total_bonus = center.ability.extra.stored_card.ability.perma_bonus
            end
            if center.ability.extra.stored_card.config.center.key == "m_stone" then
                return {vars = {'Stone Card +' .. total_bonus}}
            else
                return {vars = {rank_suffix .. ' of ' .. center.ability.extra.stored_card.base.suit .. ' +' .. total_bonus}}
            end
        end
    end,
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers and not context.blueprint then
            local eval = function() return (G.GAME.current_round.hands_played == 0 and card.ability.extra.storing == false and not card.debuff) end
            juice_card_until(card, eval, true)
        end

        if context.before and #context.full_hand == 1 and card.ability.extra.storing == false and not context.full_hand[1].debuff and G.GAME.current_round.hands_played == 0 and not card.debuff and not context.blueprint then
            card.ability.extra.stored_card = copy_card(context.full_hand[1], nil, nil, G.playing_card)
            card.ability.extra.stored_card.states.visible = nil

            context.full_hand[1].bag_destroying = true
            local dead_card = context.full_hand[1]
            if dead_card.ability.name == 'Glass Card' then 
                dead_card:shatter()
            else
                dead_card:start_dissolve()
            end
            table.remove(context.full_hand, 1)
            table.remove(context.scoring_hand, 1)

            card.ability.extra.storing = true
            return {message = "Card stored",}
        end

        if context.destroy_card and context.destroy_card.bag_destroying then
            return { remove = true }
        end

        if context.end_of_round and context.main_eval and card.ability.extra.storing == true and not card.debuff then
            card.ability.extra.stored_card.ability.perma_bonus = card.ability.extra.stored_card.ability.perma_bonus + 15
            return {message = "Stored card upgraded",}
        end

        if context.selling_self and card.ability.extra.storing == true and not card.debuff then
            local backpack_card = copy_card(card.ability.extra.stored_card, nil, nil, G.playing_card)
            backpack_card.played_this_ante = false
            backpack_card:add_to_deck()
            G.deck.config.card_limit = G.deck.config.card_limit + 1
            table.insert(G.playing_cards, backpack_card)
            G.deck:emplace(backpack_card)
            backpack_card.states.visible = nil
            G.E_MANAGER:add_event(Event({
                func = function()
                    backpack_card:start_materialize()
                    return true
                end
            }))
            SMODS.calculate_context({playing_card_added = true, cards = {backpack_card}})
        end
    end,
}
