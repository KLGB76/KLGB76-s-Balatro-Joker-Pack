...
SMODS.Joker{
    ...
    calculate = function(self, card, context)
        if context.end_of_round and not context.game_over and context.cardarea ~= G.hand and #G.hand.cards > 0 then
...
SMODS.Joker{
    ...
    calculate = function(self, card, context)
        ...
        if context.joker_main and card.ability.extra.AddChips > 0 then
            return {
...
SMODS.Joker{
    ...
    calculate = function(self, card, context)
        ...
        if context.before and next(context.poker_hands['Three of a Kind']) and G.GAME.current_round.hands_played == 0 then
...
SMODS.Joker{
    ...
    loc_txt = { -- local text
        ...
        text = {
          ...
          'This Joker gains {X:mult,C:white}X0.5{} Mult per card destroyed',
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers then
            local eval = function() return (G.GAME.current_round.hands_played == 0 and #G.consumeables.cards > 0) end
            juice_card_until(card, eval, true)
        end

        if context.before and #context.full_hand == 1 and context.full_hand[1]:get_id() == 3 and not context.full_hand[1].debuff and G.GAME.current_round.hands_played == 0 then
            if #G.consumeables.cards > 0 then
                card.ability.extra.Xmult = card.ability.extra.Xmult + #G.consumeables.cards * 0.5
...
--Polycarbonate Joker [ON HOLD] (might need lovely patch)
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers then
            ...
            for k, v in pairs(G.playing_cards) do --Looks for Negative in Full Deck
                if v.edition and v.edition.key == 'e_negative' then card.ability.extra.Xmult = card.ability.extra.Xmult + 0.5 end
            end
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers then
            ...
            for k, v in pairs(G.jokers.cards) do --Looks for Driver's License
                if v.ability.name == "Driver's License" then 
        ...
        if context.before and card.ability.extra.has_license == false then
            for i=1, #context.full_hand do
                local card_is_scoring = false
                for j=1, #context.scoring_hand do
                    if context.full_hand[i] == context.scoring_hand[j] then
...
SMODS.Joker{
    ...
    calculate = function(self, card, context)
        ...
        if context.setting_blind and card.ability.extra.has_seance == false and not card.getting_sliced then
        ...
        if context.joker_main and card.ability.extra.Xmult > 1 and card.ability.extra.has_seance == true then
            return {
                card = card,
                Xmult_mod = card.ability.extra.Xmult,
                message = 'X' .. card.ability.extra.Xmult .. ' Mult',
...
SMODS.Joker{
    ...
    loc_vars = function(self,info_queue,center)
        info_queue[#info_queue+1] = { set = 'Other', key = 'my_key' } --adds "Edible"'s description next to this card's description
        return {vars = {center.ability.extra.Xmult}} --#1# is replaced with card.ability.extra.Xmult
    end,
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers then
            ...
            for k, v in pairs(G.jokers.cards) do --Looks for food in Jokers
                if v.ability.name == "Gros Michel" or v.ability.name == "Egg" or v.ability.name == "Ice Cream" or v.ability.name == "Cavendish" or v.ability.name == "Turtle Bean" or v.ability.name == "Popcorn" or v.ability.name == "Ramen" or v.ability.name == "Seltzer" then
...
SMODS.Joker{
    ...
    calculate = function(self, card, context)
        ...
        if context.setting_blind and card.ability.extra.chicken_first == true and not card.getting_sliced then
        ...
        if context.joker_main and card.ability.extra.AddMult > 0 and card.ability.extra.chicken_first == false then
            return {
                card = card,
...
-- Known Bugs: All cards randomized by Metronome get debuffed by The Pillar
